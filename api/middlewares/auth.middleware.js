...
export async function validateToken(req, res, next) {
	try {
		...
		jwt.verify(token, process.env.JWT_SECRET, function (err, decoded) {
			// decoded contient les données du token si le token est valide
			// Si le token ne possède pas une propriété user_id, c'est qu'il est invalide
			if (!decoded?.user_id) throw new HttpError('Token invalide', 401);
		...
		next(new HttpError("Accès non autorisé", 401));
	}
